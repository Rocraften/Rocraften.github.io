...
function calculateCosineSimilarity(vecA, vecB) {
    ...
    try {
        ...
        const magnitudeA = vecA.norm().dataSync()[0];
        const magnitudeB = vecB.norm().dataSync()[0];
        if (magnitudeA > 0 && magnitudeB > 0) {
            similarity = dotProduct / (magnitudeA * magnitudeB);
        }
    } catch (error) {
        console.error("Error calculating similarity:", error);
        similarity = 0; // Default to 0 on error
    }
    return similarity;
}
...
async function getIntentWithUSE(query) {
    ...
    const queryEmbedding = model.embed([query]); // Start embedding user query

    const intentEmbeddings = {}; // Cache intent embeddings

    try {
        // Pre-calculate embeddings for all intent examples if not cached
        for (const intentName in intents) {
            if (!intentEmbeddings[intentName]) {
                 intentEmbeddings[intentName] = await model.embed(intents[intentName].examples);
            }
        }

        const queryVec = await queryEmbedding; // Wait for user query embedding

        // Compare query embedding with all example embeddings
        for (const intentName in intents) {
            const examplesTensor = intentEmbeddings[intentName];
            for (let i = 0; i < intents[intentName].examples.length; i++) {
                 // tf.tidy helps manage memory by disposing intermediate tensors
                 const similarity = tf.tidy(() => {
                    const exampleVec = examplesTensor.slice([i, 0], [1]); // Extract the i-th example embedding
                    return calculateCosineSimilarity(queryVec, exampleVec);
                 });
        ...
        queryVec.dispose(); // Clean up query tensor
        ...
        // Clean up cached intent embeddings tensors
        for (const intentName in intentEmbeddings) {
             intentEmbeddings[intentName]?.dispose();
        }
        queryEmbedding?.dispose(); // Ensure query embedding is disposed even on error
    }
}
